 ...
 * ¿Por qué? Porque @JsonProperty fuerza a que el JSON entrante tenga UN solo nombre exacto, 
 * lo que hacía chocar la entrada manual del dashboard cuando Luis llenaba el formulario. 
 * * @JsonAlias actúa como un "embudo bilingüe": le permite a Spring Boot aceptar 
 * tanto el formato con guiones que envía WordPress Contact Form 7 ("tu-email") 
 * como el formato limpio que envía nuestro Frontend de Administración ("email").
...
public class PedidoWebhookDTO {
    ...
    @Schema(description = "Nombre del cliente", example = "Efrén Tovar Silva")
    private String nombre;

    @JsonAlias({"tu-email", "email"})
    @Schema(description = "Email del cliente (Identificador Único y Llave Maestra)", example = "ejts29@gmail.com")
    private String email;

    @JsonAlias({"tu-telefono", "telefono"})
    @Schema(description = "Teléfono de contacto (Opcional)", example = "+56952480417")
